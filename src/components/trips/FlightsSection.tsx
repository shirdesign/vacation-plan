...
export default function FlightsSection({
  ...
  async function addFlight(e: React.FormEvent) {
    ...
    if (flight) {
      ...
      setForm({ from_location: '', to_location: '', flight_date: '', depart_time: '', airline: '', flight_number: '', price: '', is_booked: false, notes: '' })
      setShowForm(false)
    }
    setSaving(false)
  }
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-6">
      ...
      {open && (
        <div className="mt-4 space-y-3">
          ...
          {showForm ? (
            <form onSubmit={addFlight} className="bg-indigo-50 border border-indigo-200 rounded-2xl p-4 space-y-3">
              ...
              <div className="flex gap-2">
                <button type="submit" disabled={saving} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 rounded-xl transition text-sm disabled:opacity-50">
                  {saving ? 'שומר...' : 'הוסיפי טיסה'}
                </button>
                <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 border border-gray-300 text-gray-600 rounded-xl hover:bg-gray-50 transition text-sm">
                  ביטול
                </button>
              </div>
            </form>
